...
class StandaloneRunner(Runner):
    ...
    def call_initialize(self, *ids_list):
        ...
        output = self.__run_standalone(
            *ids_list, method_name="init", code_parameters=code_parameters
        )
        return output
    ...
    def call_main(self, *ids_list):
        code_parameters = self.get_code_parameters()
        output = self.__run_standalone(
            *ids_list, method_name="main", code_parameters=code_parameters
        )
        return output

    def call_finalize(self, *ids_list):
        output = self.__run_standalone(
            *ids_list, method_name="finalize", code_parameters=None
        )
        return output
    ...
    def __run_standalone(self, *ids_list, method_name, code_parameters=None):
        ...
        arg_metadata_list = self._actor.code_description["implementation"][
            "subroutines"
        ][method_name].get("arguments")
        ...
        full_command = cmdln_utlis.resolve_cmd_tags(
            full_command, exec_cmd, runtime_settings
        )
        ...
        results = self._binder.run_standalone(
            *ids_list,
