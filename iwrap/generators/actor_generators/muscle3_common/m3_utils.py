...
def copy_native_lib(install_dir: str, project_settings: dict, destination_dir: str):
    ...
    native_lib_path = (
        project_settings["code_description"].get("implementation", {}).get("code_path")
    )
...
def copy_build_info(install_dir: str, project_settings: dict):
    output_dict = dict(
        ...
        if key in ["actor_description", "code_description", "build_info"]
    )
    with open(
        f"{install_dir}/{project_settings['actor_description']['actor_name']}.yaml", "w"
    ) as file:
...
def validate(project_settings: dict = None):
    ...
    if init_arguments or finalize_arguments:
        raise ValueError(
            "MUSCLE3 actor generator cannot handle INIT/FINALIZE methods with IDS arguments"
        )
