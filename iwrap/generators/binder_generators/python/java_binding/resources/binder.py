...
class LanguageBinder(Binder):
    ...
    def __get_wrapper_library(self):
        ...
        code_file = self.actor.code_description["implementation"]["code_path"].split(
            "/"
        )[-1]
    ...
    def __get_ids_native(self, arg_metadata_list):
        ...
        for arg_meta_data in arg_metadata_list:
            ids_java = self.ids_converter.prepare_native_type(
                JavaIDSDescription, arg_meta_data["type"]
            )
