...
class WrapperGeneratorRegistry:
    ...
    def initialize(cls):
        cls.__generators = utils.discover_generators(
            cls.__builtin_pkg_name, cls.__plugin_pkg_name, WrapperGenerator
        )
    ...
    def get_generator(
        ...
        for generator in cls.__generators:
            if (
                code_language.lower() == generator.code_language.lower()
        ...
        raise ValueError(
            f'ERROR: No generator found to wrap code language "{code_language}" for type "{type}"! \n'
