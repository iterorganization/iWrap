...
class FortranWrapperGenerator(WrapperGenerator):
    ...
    def validate(self, project_settings: dict):
        include_path = (
            project_settings["code_description"]
            .get("implementation", {})
            .get("include_path")
        )
        ...
        if not Path(include_path).exists():
            raise ValueError(f"Path to module file is not valid! {str(include_path)}")
    ...
    def initialize(self, project_settings: dict):
        ...
        self.install_dir: str = str(
            Path(
                install_dir,
                ProjectSettings.get_settings().actor_description.actor_name,
                self.wrapper_dir,
            )
        )
    ...
    def generate(self, project_settings: dict):
        ...
        process_template_dir(
            "iwrap.generators.wrapper_generators.fortran_wrapper",
            "resources",
            self.install_dir,
            project_settings,
            filter_func=None,
    ...
    def __copy_native_lib(self, project_settings: dict):
        ...
        native_lib_path = project_settings["code_description"]["implementation"][
            "code_path"
        ]
    ...
    def __copy_include(self, project_settings: dict):
        ...
        include_path = project_settings["code_description"]["implementation"][
            "include_path"
        ]
