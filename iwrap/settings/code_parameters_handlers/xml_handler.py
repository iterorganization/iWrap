...
class XMLHandler(GenericHandler):
    ...
    def __get_xml_value(self, splitted_path, tree):
        ...
        if len(splitted_path) == 1:
            if current_path_without_index == tree.tag:
                if self.__is_leaf(tree):
                    ...
                    raise Exception(
                        f"Node <{tree.tag}> is not a leaf node, thus it cannot be last element in provided path"
                    )

        # if there are more elements in splitted path AND current node is a leaf
        elif self.__is_leaf(tree):
            raise Exception(
                f'Node <{tree.tag}> is already XML leaf, but remaining path "{rest_of_splitted_path}" contains child nodes'
            )
        ...
        # if there are more elements in splitted path AND current node is NOT a leaf
            ...
            if index + 1 > len(found_nodes):
                raise Exception(
                    f"XML node was not found. Searched node <{current_path_without_index}>, found {len(found_nodes)} nodes, tried to access index [{index}]"
                )
    ...
    def __set_xml_value(self, splitted_path, tree, value):
        ...
        if len(splitted_path) == 1:
            if splitted_path[0] == tree.tag:
                if self.__is_leaf(tree):
                    ...
                    raise Exception(
                        f"Node <{tree.tag}> is not a leaf node, thus it's value cannot be set.)"
                    )
        ...
        return self.__set_xml_value(
            rest_of_splitted_path, tree.findall(rest_of_splitted_path[0])[index], value
        )
    ...
    def get_parameter(self, path_to_node: str) -> str:
        ...
        if path_root != root.tag:
            raise Exception(
                f'Path root tag does not match XML document root. Path root is "{path_root}", while document root is "{root.tag}"'
            )
