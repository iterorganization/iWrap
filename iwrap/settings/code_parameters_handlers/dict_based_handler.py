...
class DictBasedHandler(GenericHandler, ABC):
    ...
    def _get_dict_value(self, splitted_path, parameters_dict):
        ...
        current_path_without_index, index, rest_of_splitted_path = (
            self._extract_path_info(splitted_path)
        )

        try:
            if index is not None:
                if not isinstance(
                    dict[current_path_without_index], collections.abc.Sequence
                ):
                    raise IndexError(
                        f"Parameters node {dict[current_path_without_index]} is not an array and cannot be accessed using array index."
                    )
                return self._get_dict_value(
                    rest_of_splitted_path,
                    parameters_dict[current_path_without_index][index],
                )
            else:
                return self._get_dict_value(
                    rest_of_splitted_path, parameters_dict[current_path_without_index]
                )
        except KeyError:
            self.__logger.error(
                f"KeyError. Cannot find {current_path_without_index[0]} key in parameters document."
            )
    ...
    def _set_dict_value(self, splitted_path, parameters_dict, value):
        ...
        if len(rest_of_splitted_path) == 0:
            if index is not None:
                if not isinstance(
                    parameters_dict[current_path_without_index],
                    collections.abc.Sequence,
                ):
                    raise IndexError(
                        f"Parameters node {parameters_dict[current_path_without_index]} is not an array and cannot be accessed using array index."
                    )
                try:
        ...
        # if there are more nodes in path just jump along path
            ...
            try:
                ...
                self.__logger.error(
                    f"KeyError. Cannot find {splitted_path[0]} key in parameters document."
                )
