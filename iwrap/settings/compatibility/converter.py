...
class Converter:
    ...
    def __delete(self, splitted_path, condition, current_node=None):
        ...
        if len(splitted_path) == 1:
            if condition is None:
                ...
                if isinstance(current_node[splitted_path[0]], list):
                    condition = condition.replace(
                        "$TARGET", "current_node[splitted_path[0]][x]"
                    )
                    del_indexes = []
                    ...
                    condition = condition.replace(
                        "$TARGET", "current_node[splitted_path[0]]"
                    )
                    if eval(condition):
                        try:
                            del current_node[splitted_path[0]]
            ...
            if isinstance(new_current_node, list):
                ...
                self.__delete(
                    splitted_path[1:], condition, current_node=new_current_node
                )
    ...
    def __set(
        self, splitted_path, value, condition, current_node=None, create_node=True
    ):
        """
        Sets value in dict.
        Args:
        ...
        if len(splitted_path) == 1:
            ...
            return self.__set(
                ...
                value,
                condition,
                current_node=current_node[splitted_path[0]],
            )
    ...
    def convert(yaml_dict, command_line=False):
        ...
        for mapping in mappings:
            ...
            if "command" in mapping.keys() and "actions" in mapping.keys():
                converter.__logger.error(
                    '[ERROR]: "command" and "actions" keywords cannot be used in the same mapping.'
                )
        ...
        if not command_line and transform_counter > 0:
            converter.__logger.warning(
                "[WARNING]: You are using outdated code description."
...
def main():
    ...
    parser.add_argument(
        "-i",
        "--in-place",
        help="Input is renamed as <input>_old. Output is saved under input's filename.",
        action="store_true",
    )
    ...
    if args.in_place:
        ...
        rename(
            input_filename,
            splitext(input_filename)[0] + "_old" + splitext(input_filename)[1],
        )
    elif args.output:
