...
def get_config_file_for_domain():
    ...
    for key, value in known_hosts.items():
        ...
        __logger.warning(
            f'No specific config for "{domain_fqdn}". Default config file is used.'
        )
    ...
    if not config_file_path.exists():
        __logger.warning(
            f'File "{config_file_name}" not found. Default config file is used.'
        )
        config_file_path = Path(
            iwrap.IWRAP_DIR, "resources/config", "config_default.yaml"
        )
