...
def discover_generators(
    builtin_pkg_name: str, plugin_pkg_name: str, generator_base_class
):
    generators_list = []
    try:
        generator_module = importlib.import_module(builtin_pkg_name)
        packages = pkgutil.walk_packages(
            generator_module.__path__, generator_module.__name__ + "."
        )
        ...
        __logger.exception(
            f"No built-in {generator_base_class.__name__} plug-ins have been loaded:\n{exc}"
        )
    except Exception:
        __logger.exception(
            f'Error while loading built-in {generator_base_class.__name__} plug-in {name}"'
        )
        raise
    ...
    try:
        ...
        __logger.info(
            f"No external {generator_base_class.__name__} plug-ins have been found"
        )
