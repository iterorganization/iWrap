...
class CodeParametersPane(ttk.Frame, IWrapPane):
    ...
    def __init__(self, master=None):
        ...
        ttk.Label(self, text="Parameters file:").grid(
            column=0, row=0, padx=10, pady=(15, 5), sticky=(tk.W, tk.N)
        )
        self.parameters_path_entry = ttk.Entry(self, textvariable=self.parameters_path)
        self.parameters_path_entry.grid(
            column=1, row=0, padx=10, pady=(15, 5), sticky=(tk.W, tk.E)
        )
        ttk.Button(
            self,
            text="Browse...",
            command=lambda: self.on_click_file(self.parameters_path),
            width=10,
        ).grid(column=2, row=0, padx=10, pady=(15, 5))
        ToolTip(self.parameters_path_entry, "parameters_file")

        # SCHEMA PATH AND BROWSE BUTTON
        ttk.Label(self, text="Schema file:").grid(
            column=0, row=1, padx=10, pady=5, sticky=(tk.W, tk.N)
        )
        self.schema_path_entry = ttk.Entry(self, textvariable=self.schema_path)
        self.schema_path_entry.grid(
            column=1, row=1, padx=10, pady=5, sticky=(tk.W, tk.E)
        )
        ttk.Button(
            self,
            text="Browse...",
            command=lambda: self.on_click_file(self.schema_path),
            width=10,
        ).grid(column=2, row=1, padx=10, pady=5)
        ...
        self.format_combobox = ttk.Combobox(
            self, state="readonly", textvariable=self.parameters_format
        )
        ...
        validate_button = ttk.Button(
            self, text="Validate", command=self.validate_parameters
        )
        validate_button.grid(
            column=1, row=3, padx=10, pady=(15, 5), sticky=(tk.W, tk.E)
        )
    ...
    def validate_parameters(self):
        ...
        if self.parameters_path.get() == "" or self.schema_path.get() == "":
            messagebox.showerror(
                "WARNING! - Validation Error", "Validation aborted:\n-EMPTY PATH-"
            )
            return
        ...
        try:
            parameters_handler: ParametersHandlerInterface = HandlerFactory.get_handler(
                self.parameters_format.get()
            )
            parameters_handler.initialize(
                parameters_path=utils.resolve_path(
                    self.parameters_path.get(),
                    ProjectSettings.get_settings().root_dir_path,
                ),
                schema_path=utils.resolve_path(
                    self.schema_path.get(), ProjectSettings.get_settings().root_dir_path
                ),
            )
            ...
            messagebox.showerror(
                "Validation Error",
                f"The process encountered an error. Verify the input files!\n\n"
                f"{message}",
            )
        else:
            # A message box with information about the validation result.
            messagebox.showinfo("Verification done", "Validation passed")
    ...
    def update_settings(self):
        ...
        code_description.implementation.code_parameters.format = (
            self.parameters_format.get()
        )
