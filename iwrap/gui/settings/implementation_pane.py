...
class ImplementationPane(ttk.Frame, IWrapPane):
    ...
    def __init__(self, master=None):
        ...
        labelframe = ttk.LabelFrame(
            self, text="Implementation", borderwidth=2, relief="groove"
        )
        labelframe.pack(fill=tk.BOTH, side=tk.TOP, expand=0, anchor=tk.NW, pady=10)
        labelframe.grid_columnconfigure(1, weight=1)

        # LANGUAGE COMBOBOX
        ttk.Label(labelframe, text="*Programming language:").grid(
            column=0, row=1, padx=10, pady=5, sticky=(tk.W, tk.N)
        )
        self.programming_language_combobox = ttk.Combobox(labelframe, state="readonly")
        available_code_languages = ["None"] + list(
            Engine().active_generator.code_languages
        )
        self.programming_language_combobox["values"] = available_code_languages
        self.programming_language_combobox.current(0)
        self.programming_language_combobox.grid(
            column=1, row=1, padx=10, pady=5, sticky=(tk.W, tk.E)
        )
        ToolTip(self.programming_language_combobox, "programming_language")

        # DATA TYPE COMBOBOX
        ttk.Label(labelframe, text="*Data type:").grid(
            column=0, row=2, padx=10, pady=5, sticky=(tk.W, tk.N)
        )
        ...
        self.data_type_combobox.grid(
            column=1, row=2, padx=10, pady=5, sticky=(tk.W, tk.E)
        )
        ToolTip(self.data_type_combobox, "data_type")

        # ENTRY FOR IMAS COMPLIANT VERSION
        ttk.Label(labelframe, text="*Data Dictionary compliant:").grid(
            column=0, row=3, padx=10, pady=5, sticky=(tk.W, tk.N)
        )
        self.data_dictionary_compliant_entry = ttk.Entry(
            labelframe, textvariable=self.data_dictionary_compliant
        )
        self.data_dictionary_compliant_entry.grid(
            column=1, row=3, padx=10, pady=5, sticky=(tk.W, tk.E)
        )
        ToolTip(self.data_dictionary_compliant_entry, "data_dictionary_compliant")

        # BROWSE BUTTON AND ENTRY FOR CODE PATH
        ttk.Label(labelframe, text="*Code path:").grid(
            column=0, row=5, padx=10, pady=5, sticky=(tk.W, tk.N)
        )
        ...
        ttk.Button(
            labelframe,
            text="Browse...",
            command=lambda: self.on_click_file(self.code_path),
            width=10,
        ).grid(column=2, row=5, padx=10, pady=5)
        ToolTip(self.code_path_entry, "code_path")

        # BROWSE BUTTON AND ENTRY FOR ROOT DIR
        ttk.Label(labelframe, text="Root dir:").grid(
            column=0, row=4, padx=10, pady=5, sticky=(tk.W, tk.N)
        )
        ...
        ttk.Button(
            labelframe, text="Browse...", command=self.on_click_dir, width=10
        ).grid(column=2, row=4, padx=10, pady=5)
        ToolTip(self.root_dir_entry, "root_dir")

        # MODULE PATH
        self.include_path = tk.StringVar()
        self.include_path.set(
            ProjectSettings.get_settings().code_description.implementation.include_path
            or ""
        )
        ttk.Label(labelframe, text="*Include path:").grid(
            column=0, row=6, padx=10, pady=5, sticky=(tk.W, tk.N)
        )
        browse_button = ttk.Button(
            labelframe,
            text="Browse...",
            command=lambda: self.on_click_file(self.include_path),
            width=10,
        )
    ...
    def update_settings(self, *args):
        ...
        code_description.implementation.programming_language = (
            self.programming_language_combobox.get()
        )
        ...
        code_description.implementation.data_dictionary_compliant = (
            self.data_dictionary_compliant.get()
        )
    ...
    def reload(self):
        ...
        if programming_language not in [x.lower() for x in available_code_languages]:
            messagebox.showwarning(
                "Warning",
                "Unknown programming language. The programming language set to 'None'",
            )
        ...
        if self.data_type not in self.data_type_combobox["values"]:
            ...
            self.data_type not in self.data_type_combobox["values"]
        ...
        self.data_dictionary_compliant.set(
            code_description.implementation.data_dictionary_compliant or ""
        )
