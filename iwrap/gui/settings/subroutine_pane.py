...
class SubroutinePane(tkinter.ttk.Frame, IWrapPane):
    ...
    def __init__(self, method_type, master=None):
        ...
        self.subroutine = getattr(
            ProjectSettings.get_settings().code_description.implementation.subroutines,
            method_type,
        )
        ...
        labelframe.pack(
            fill=tkinter.BOTH, side=tkinter.TOP, expand=1, anchor=tkinter.NW, pady=10
        )
        labelframe.grid_columnconfigure(1, weight=1)
        # INIT / MAIN / FINALIZE
        tkinter.ttk.Label(labelframe, text="Name:").grid(
            column=0, row=1, padx=10, pady=5, sticky=(tkinter.W, tkinter.N)
        )
        ...
        tkinter.ttk.Label(labelframe, text="Need code parameters:").grid(
            column=0, row=2, padx=10, pady=5, sticky=(tkinter.W, tkinter.N)
        )
        self.code_params_check = tkinter.Checkbutton(
            labelframe, variable=self.need_code_parameters
        )
        self.code_params_check.grid(
            column=1, row=2, padx=10, pady=5, sticky=(tkinter.W, tkinter.N)
        )
        ### ARGUMENTS ###
        # LABEL FRAME
        labelframe = tkinter.ttk.LabelFrame(
            self, text="Arguments", borderwidth=2, relief="groove"
        )
        ...
        table_frame = tkinter.Frame(
            main_content_frame, highlightbackground="black", highlightthickness=1
        )
        ...
        buttons_frame.pack(
            fill=tkinter.BOTH, side=tkinter.RIGHT, expand=0, padx=3, pady=3
        )
        buttons_frame_center = tkinter.ttk.Frame(buttons_frame)
        buttons_frame_center.place(
        ...
        edit_button = tkinter.ttk.Button(
            buttons_frame_center, text="Edit...", state="disabled", width=10
        )
        edit_button.pack(side=tkinter.TOP, expand=1, pady=10)
        buttons.append(edit_button)
        up_button = tkinter.ttk.Button(
            buttons_frame_center, text="Up", state="disabled", width=10
        )
        up_button.pack(side=tkinter.TOP, expand=1)
        buttons.append(up_button)
        down_button = tkinter.ttk.Button(
            buttons_frame_center, text="Down", state="disabled", width=10
        )
        down_button.pack(side=tkinter.TOP, expand=1)
        buttons.append(down_button)
        remove_button = tkinter.ttk.Button(
            buttons_frame_center, text="Remove", state="disabled", width=10
        )
