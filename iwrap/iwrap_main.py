...
def get_parser(is_commandline_mode: bool) -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="iWrap - a modular IMAS actor generator, used for creating "
    ...
    generation_group.add_argument(
        "-a", "--actor-name", type=str, help="user defined name of the actor"
    )

    generation_group.add_argument(
        "-t", "--actor-type", type=str, help="type of the actor to be generated"
    )
...
def main(argv: List[str] = sys.argv[1:], is_commandline_mode=True) -> int:
    ...
    if args.list_actor_types:
        ...
        for generator in Engine().registered_generators:
            print(
                generator.type.center(20),
                ":",
                generator.name.center(30),
                ":",
    ...
    if args.list_actor_details:
        ...
        print(
            "Supported IDS types (actor)",
            "\t\t:\t",
            ", ".join(generator.actor_data_types),
        )
        print(
            "Supported languages (code/wrapper)",
            "\t:\t",
            ", ".join(generator.code_languages),
        )
        print(
            "Supported IDS types (code/wrapper)",
            "\t:\t",
            ", ".join(generator.code_data_types),
        )
        print("-" * 80)
...
if __name__ == "__main__":
    ...
    main(
        [
            ...
            "legacy",
            "-f",
            f"../examples/{actor_name}/{actor_name}.yaml",
        ],
