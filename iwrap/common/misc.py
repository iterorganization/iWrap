...
class Dictionarizable(ABC):
    ...
    def _list_attributes(self):
        members_list = inspect.getmembers(
            self, lambda member: not inspect.isroutine(member)
        )
        members_list = filter(
            lambda member: not member[0].startswith("_"), members_list
        )
        ...
        for key, value in self._list_attributes():
            if isinstance(value, Dictionarizable):
                dict_[key] = value.to_dict(
                    resolve_path, make_relative, project_root_dir
                )
            elif isinstance(value, List):
                dict_[key] = [
                    item.to_dict(resolve_path, make_relative, project_root_dir)
                    if isinstance(item, Dictionarizable)
                    else item
                    for item in value
                ]
            else:
...
class CustomDumper(yaml.SafeDumper):
    ...
    def represent_sequence(self, tag, sequence, flow_style=None):
        if not sequence:  # Check if the sequence is empty
            return super().represent_scalar("tag:yaml.org,2002:null", "")
